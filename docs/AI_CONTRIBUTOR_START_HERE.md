...
| Authoritative-world foundation | **Foundation proof passed; returning-player proof remains open.** Migration 007 security hardening is recorded on isolated `dealt-world-proof-staging` as `20260925060458 / staging_saved_game_security_hardening_007`, and read-only catalog checks confirm its intended grant/RLS effects. Do not replay it. Production remains intentionally unchanged. | Issue #175; PR #165; `docs/AUTHORITATIVE_WORLD_STAGING_PROOF.md`; `docs/SAVED_GAME_READINESS_2026-09-25.md`; `backend/supabase/migrations/005_*`, `006_*`, `007_*` |
...
|---:|---|---|---|
| P0 | Complete the **authenticated saved-game proof** in the named non-production target before considering external testers: preserve the recorded migration-007 apply state, prove two-user Auth/RLS/direct-write isolation, and verify one canonical Flask returning-player loop across reload and a same-account second session on desktop and 375×812. | `feature/authenticated-saved-game-proof` | Do not run `db push`, replay migration 007, repair migration history, change production, or add a second writer. Preserve demo-ledger isolation and idempotent receipts. |
