...
- Added `PlayableMap.test.tsx` coverage for timeout recovery, pre-load connection error, retry rebuild, tactical fallback callback, and the rule that a later tile error does not replace a successfully loaded usable map. Full validation: Vitest 733/733 across 55 files, TypeScript clean, asset audit clean, and production build passed. The persistent build-size warnings remain a separate optimization backlog item.
