...
### Grid is Gameplay Truth; Recon is Read-Only Visuals
The **8×8 block grid** (generated by `grid_generator.py`) is the authoritative game state used for all gameplay mechanics — combat, income, cover calculations. Visual "Recon" views (top-down Mapbox satellite, Street View, 3D Tiles) are **read-only overlays** that render the real-world geography of a claimed block. They never affect game mechanics.

### Canonical API Client
- **`api.service.ts`** (axios-based) — preferred for all domain-specific endpoint calls (auth, blocks, combat, inventory, world).
- **`apiClient.ts`** (fetch-based) — used by `useApi.ts` hook and `syncService.ts` for generic/offline-queued requests.
- **`socket.service.ts`** — canonical real-time service. `SocketService.ts` re-exports from it.

### Auth Middleware
All Flask blueprints use **`middleware.auth.require_auth`** (single source of truth). In dev mode (no `SUPABASE_URL`), a mock `DEV_USER` is injected automatically. In production, Supabase JWT tokens are validated server-side using the `service_role` key.

> **Security**: `SUPABASE_SERVICE_ROLE_KEY` is only loaded server-side. Never use it in `VITE_*` env vars.
